...
class MutableSpanTests : XCTestCase {
  ...
  func testDroppingAndAddingAttributes() {
    ...
    // Test that we have the newest re-added initial  gentries.
  ...
  func testAsSpanData() {
    ...
    let mutableSpan = MutableSpan(span: readableSpan)!
  ...
  private func createTestSpan(config: SpanLimits) -> MutableSpan {
    return createTestSpan(kind: .internal, config: config, parentContext: nil, attributes: [String: AttributeValue]())
  }

  private func createTestSpan(kind: SpanKind, config: SpanLimits, parentContext: SpanContext?, attributes: [String: AttributeValue]) -> MutableSpan {
    ...
    let span = RecordEventsReadableSpan.startSpan(context: spanContext,
                                                  ...
                                                  totalRecordedLinks: 1,
                                                  startTime: startTime)
    let mutableSpan = MutableSpan(span: span)

    XCTAssertEqual(spanProcessor.onStartCalledTimes, 1)
    return mutableSpan!
  }

  private func spanDoWork(span: MutableSpan, status: Status) {
