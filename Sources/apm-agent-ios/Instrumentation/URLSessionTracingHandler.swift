...
//
        ...
        span.putHttpStatusCode(statusCode: httpResponseStatusCode, reasonPhrase: resourceCompletion.error?.localizedDescription ?? "" )
            ...
            span.status = .error
            span.addEvent(name: "error",
                          attributes: ["code" : AttributeValue.int(error.errorCode),
                                       "domain": AttributeValue.string(URLError.errorDomain),
                                       "cause": AttributeValue.string(error.localizedDescription)],
