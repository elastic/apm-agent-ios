...
class ApplicationResourceProviderTests : XCTestCase {
    func testContents() {
        ...
        let provider = ApplicationResourceProvidier(source: appData) as ResourceProvider
        
        let resource = provider.create()
        
        XCTAssertTrue(appData.name == resource.attributes[ResourceConstants.serviceName.rawValue]?.description)
        let versionDescription = resource.attributes[ResourceConstants.serviceVersion.rawValue]?.description
